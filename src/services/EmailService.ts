...
import { SendMailFormBody } from '../routes/index';
import { OneDriveAPIDao } from '../dal/OneDriveAPIDao';
...
export class EmailService {
  ...
  public static async prepForEmailMessage(accessToken: string, displayName: string, destinationEmailAddress: string) {

    // Get the current user's profile photo.
    const profilePhoto = await OneDriveAPIDao.getProfilePhoto(accessToken);

    return EmailService.generateMailBody(displayName, destinationEmailAddress, 'link.webUrl', profilePhoto)
    /*
    // Upload profile photo as file to OneDrive.
    return graphHelper.uploadFile(accessToken, profilePhoto)
      .then((file) => graphHelper.getSharingLink(accessToken, file.id))   // Get sharingLink for file.
      .then((link) => emailer.generateMailBody(displayName, destinationEmailAddress, link.webUrl, profilePhoto) );
    */
   ...
   */
  private static getEmailContent(name, sharingLink) {
   ...
   */
  private static wrapEmail(content, recipient, file) {
    const attachments = [{
   ...
   */
  private static generateMailBody(name, recipient, sharingLink, file) {
    return EmailService.wrapEmail(EmailService.getEmailContent(name, sharingLink), recipient, file);
  }
}